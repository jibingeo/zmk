...
static int iqs5xx_sample_fetch (const struct device *dev) {
    ...
    if(data->raw_data.finger_count) {
      //LOG_INF("trackpad result %d", data ->raw_data.finger_count);
      //LOG_INF("trackpad result %d,%d", data->raw_data.fingers[0].ax,data->raw_data.fingers[0].ay);
      // LOG_INF("trackpad result %d, %d", data->raw_data.fingers[0].strength, data->raw_data.fingers[0].area);
    }
